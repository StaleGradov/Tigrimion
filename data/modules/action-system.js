...
class ActionSystem {
    constructor(mapSystem) {
        ...
        this.actionConfigs = {
            ...
            'hunt': {
                ...
                always_monster: true,
                isComplexAction: true // Флаг для сложных действий с собственной логикой
            },
            'hunt_caravan': {
                icon: '🏹',
    ...
    async loadHuntModule() {
        ...
        try {
            ...
            // Ждем немного для регистрации класса
            ...
            if (window.HuntAction) {
                ...
                throw new Error("Класс HuntAction не зарегистрирован после загрузки");
            }
        } catch (error) {
    ...
    registerModule(moduleName, moduleInstance) {
        ...
        if (moduleInstance.config && moduleInstance.config.id) {
            this.actionConfigs[moduleInstance.config.id] = {
                ...this.actionConfigs[moduleInstance.config.id],
    ...
    addResourceToHero(resourceId, resourceName, quantity, resourceType) {
        ...
        console.log(`📦 ActionSystem: Добавлен ресурс ${resourceId}: ${quantity} шт.`);
