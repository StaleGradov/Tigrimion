...
class MapSystem {
    ...
    async loadMapData() {
        try {
            ...
            if (this.tacticalMaps.length === 0 && this.localMaps.length === 0) {
                this.createTestMaps();
            }
    ...
    async loadGlobalMapFile(mapPath) {
        ...
        console.log(`🌍 Загрузка глобальной карты: ${mapPath}`);
   ...
   setStartPositions() {
    ...
    renderLocalMap() {
        // Если есть текущая локальная карта - показываем её через тактический рендерер
