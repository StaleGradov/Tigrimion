...
/**
 * Модуль для действий на карте, вероятностей, охоты и ресурсов
 */
class MapActions {
    constructor() {
        ...
        console.log("✅ MapActions инициализирован с системой действий и вероятностей");
    }
     ...
     */
    calculateActionChance(baseChance, cellKey, action) {
    ...
    /**
     * Уменьшение шанса после успешного действия
     */
    reduceActionChance(cellKey, action, reduction = 5) {
        if (!this.actionSuccessRates[cellKey]) {
            this.actionSuccessRates[cellKey] = {};
        }
    ...
    getCellByKey(cellKey) {
        ...
        if (!mapSystem?.currentTacticalMap) return null;
        return mapSystem.currentTacticalMap.cells[cellKey];
    }
    ...
    getHuntChanceForMonster(cellKey, monsterId, baseChance) {
        ...
        if (battleSystem) {
            ...
            if (monster) {
                const levelPenalty = monster.level * 2;
    ...
    saveProbabilityState() {
        try {
            const mapSystem = window.game?.systems?.map;
            const mapId = mapSystem?.currentTacticalMap?.id || 'global';
    ...
    /**
     * Сбросить вероятности для текущей карты
     */
    resetProbabilityState() {
        ...
        if (mapSystem?.currentTacticalMap?.cells) {
            Object.values(mapSystem.currentTacticalMap.cells).forEach(cell => {
                cell.explored = false;
                cell.hasAction = true;
            });
        }
    ...
    /**
     * Загрузка данных типов клеток и ресурсов
     */
    async loadCellData() {
        try {
    ...
    /**
     * Создание типов клеток по умолчанию
     */
    ...
    /**
     * Получение типа клетки по умолчанию
     */
    ...
    /**
     * Обновление интерфейса действий клетки
     */
    updateCellActionsUI(cell) {
        ...
        const mapSystem = window.game?.systems?.map;
        const isCurrentPosition = mapSystem && cell.col === mapSystem.playerTacticalPosition.x && 
                                 cell.row === mapSystem.playerTacticalPosition.y;
        const isReachable = mapSystem?.isCellReachable?.(cell) || false;
        const isExplorationCompleted = this.isExplorationCompleted(cell.row, cell.col);
        const isExplored = cell.explored === true || isExplorationCompleted;
    ...
    /**
     * Получение доступных действий для типа клетки
     */
    ...
    /**
     * Создание HTML для кнопок действий
     */
    createActionsButtonsHTML(cell, isCurrentPosition, isReachable, isExplored) {
    ...
    /**
     * Настройка обработчиков событий действий
     */
    setupActionEventListeners() {
        ...
        actionButtons.forEach(button => {
            ...
            button.addEventListener('click', (e) => {
                ...
                const mapSystem = window.game?.systems?.map;
                if (mapSystem?.performCellAction) {
                    mapSystem.performCellAction(action, row, col);
                }
            });
        });
    }
    ...
    async performCellAction(action, row, col) {
        ...
        if (action === 'stealth_movement') {
            if (!mapSystem?.isCellReachable?.(cell)) {
    ...
    /**
     * Выполнение действия с учетом вероятности
     */
        ...
        if (resourceType) {
            if (resourceType === 'gold') {
                ...
                if (mapSystem?.currentHero) {
                    ...
                    mapSystem.currentHero.gold += goldAmount;
    ...
    handleHuntActionSuccess(row, col) {
        ...
        if (mapSystem) {
            mapSystem.pendingAction = {
                ...
                row: row,
                col: col,
                cellKey: this.getCellKey(row, col),
    ...
    /**
     * Получение случайного охотничьего монстра
     */
    ...
    addResourceToHero(resourceId, resourceName, quantity, resourceType) {
        const mapSystem = window.game?.systems?.map;
        if (!mapSystem?.currentHero) return;
        
        if (!mapSystem.currentHero.resources) {
            mapSystem.currentHero.resources = {};
        }
        
        if (!mapSystem.currentHero.resources[resourceId]) {
            mapSystem.currentHero.resources[resourceId] = {
                id: resourceId,
                name: resourceName,
                count: 0,
                type: resourceType
        ...
        mapSystem.currentHero.resources[resourceId].count += quantity;
        
        console.log(`📦 Добавлен ресурс ${resourceId}: ${quantity} шт. Всего: ${mapSystem.currentHero.resources[resourceId].count}`);
    ...
    /**
     * Обновление ресурсов героя в UI
     */
    updateHeroResourcesUI(containerId = 'heroResourcesList') {
        const resourcesList = document.getElementById(containerId);
        const mapSystem = window.game?.systems?.map;
        if (!resourcesList || !mapSystem?.currentHero) return;
        
        if (!mapSystem.currentHero.resources || Object.keys(mapSystem.currentHero.resources).length === 0) {
            resourcesList.innerHTML = '<div class="no-resources" style="text-align: center; color: #94a3b8; padding: 20px;">Ресурсов пока нет</div>';
            return;
        }
        
        let resourcesHTML = '';
        Object.values(mapSystem.currentHero.resources).forEach(resource => {
    ...
    isCellReachable(cell) {
        const mapSystem = window.game?.systems?.map;
        if (!cell || !mapSystem?.playerTacticalPosition) return false;
        
        if (cell.col === mapSystem.playerTacticalPosition.x && cell.row === mapSystem.playerTacticalPosition.y) {
            return true;
        }
        
        const neighbors = mapSystem.getHexNeighbors?.(mapSystem.playerTacticalPosition.y, mapSystem.playerTacticalPosition.x) || [];
    ...
    /**
     * Отметка клетки как исследованной
     */
    markCellAsExplored(row, col) {
        const cellKey = `${col},${row}`;
        const mapSystem = window.game?.systems?.map;
        if (mapSystem?.currentTacticalMap && mapSystem.currentTacticalMap.cells[cellKey]) {
            mapSystem.currentTacticalMap.cells[cellKey].explored = true;
            mapSystem.currentTacticalMap.cells[cellKey].hasAction = false;
            mapSystem.currentTacticalMap.cells[cellKey].isSelected = false;
    ...
    /**
     * Завершение исследования клетки
     */
    ...
    canMoveToCell(row, col) {
        ...
        const mapSystem = window.game?.systems?.map;
        if (mapSystem?.isTransitionCell?.(cell)) {
            return true;
        }
        ...
        if (!mapSystem?.playerTacticalPosition) return false;
    ...
    highlightSelectedCell(cell) {
        const mapSystem = window.game?.systems?.map;
        if (!mapSystem?.currentTacticalMap) return;
        
        Object.values(mapSystem.currentTacticalMap.cells).forEach(c => {
            c.isSelected = false;
        });
    ...
    /**
     * Показ выбора цели охоты с вероятностями
     */
    ...
    /**
     * Начать охоту на конкретного монстра
     */
    ...
    startHuntBattle(row, col, monsterId, isSpecificTarget = false) {
        ...
        if (isSpecificTarget) {
            battleSystem.startBattleWithSpecificMonster(mapSystem.currentHero, monster, 'hunt');
        } else {
            battleSystem.startBattleWithMonster(mapSystem.currentHero, monsterId, 'hunt');
        }
    }
    
    /**
     * Метод для завершения охоты после боя
     */
    completeHuntAfterBattle(victory, escape = false, doubleLoot = false) {
        ...
        const mapSystem = window.game?.systems?.map;
        if (!mapSystem?.pendingAction) {
            console.warn("⚠️ Нет ожидающего действия охоты");
            return;
        }
        
        const { action, row, col, cellKey, monsterId, isSpecificTarget } = mapSystem.pendingAction;
    ...
    /**
     * Обработка случайной встречи с монстром
     */
    handleRandomMonsterEncounter(row, col) {
        console.log(`👹 Случайная встреча с монстром на клетке [${col},${row}]`);
        ...
        console.log(`⚔️ Начинаем бой со случайным монстром: ${randomMonster.name}`);
        battleSystem.startBattleWithMonster(mapSystem?.currentHero, randomMonster.id, 'random_encounter');
        
        this.showNotification(`👹 Вы привлекли внимание ${randomMonster.name}! Готовьтесь к бою!`, 'warning');
    }
    
    /**
     * Получение монстра из клетки
     */
    ...
    /**
     * Выполнение действия с ресурсом
     */
    ...
    /**
     * Расчет вероятностей для ресурсов
     */
     ...
     */
    async executeResourceGathering(action, row, col) {
        const cellKey = this.getCellKey(row, col);
        const baseChance = this.getActionChance(action, this.currentCellType);
        const currentChance = this.calculateActionChance(baseChance, cellKey, action);
        ...
        console.log(`🎲 Сбор ресурсов ${action}: ${roll.toFixed(1)}/${currentChance} - ${success ? 'УСПЕХ' : 'ПРОВАЛ'}`);
    ...
    /**
     * Обработка скрытного перемещения
     */
    handleStealthMovement(cell) {
        const mapSystem = window.game?.systems?.map;
        if (!mapSystem?.playerTacticalPosition) return;
        
        const neighbors = mapSystem.getHexNeighbors?.(mapSystem.playerTacticalPosition.y, mapSystem.playerTacticalPosition.x) || [];
        
        const availableCells = neighbors.filter(neighbor => {
            const neighborCell = mapSystem.currentTacticalMap?.cells[`${neighbor.col},${neighbor.row}`];
    ...
    /**
     * Показ выбора направления для скрытного перемещения
     */
    showStealthMovementSelection(availableCells, currentCell) {
        ...
        availableCells.forEach(targetCell => {
            const mapSystem = window.game?.systems?.map;
            const neighborHex = mapSystem?.currentTacticalMap?.cells[`${targetCell.col},${targetCell.row}`];
    ...
    attemptStealthMovement(targetRow, targetCol) {
        ...
        const mapSystem = window.game?.systems?.map;
        if (success) {
            if (mapSystem?.handlePeacefulMovement) {
                mapSystem.handlePeacefulMovement(targetCol, targetRow, null);
            }
            ...
            if (battleSystem && mapSystem) {
                mapSystem.pendingMovement = { x: targetCol, y: targetRow };
                battleSystem.startBattleWithMonster(mapSystem.currentHero, null, 'movement');
            }
        }
    }
    ...
    /**
     * Отображение реального изображения локации
     */
    ...
    /**
     * Отображение fallback изображения
     */
    ...
    /**
     * Создание контейнеров для действий
     */
...
console.log("📦 MapActions модуль загружен");
