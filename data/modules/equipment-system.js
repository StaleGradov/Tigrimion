...
class EquipmentSystem {
    ...
    addItemToInventory(itemId) {
        if (this.currentHero && !this.currentHero.inventory.includes(itemId)) {
            ...
            // Сохраняем игру после добавления предмета
    ...
    equipItem(itemId) {
        ...
        // Сохраняем игру после экипировки
        if (window.game && window.game.saveGame) {
            window.game.saveGame();
        }
    ...
    usePotion(item) {
        ...
        if (item.heal) {
            // Лечение героя
            const currentHealth = this.currentHero.currentHealth || 0;
            const maxHealth = window.game.systems.level.calculateHeroStats(this.currentHero, window.game.systems.bonus).maxHealth;
            const newHealth = Math.min(maxHealth, currentHealth + item.heal);
            this.currentHero.currentHealth = newHealth;
        ...
        // Сохраняем игру после использования
        if (window.game && window.game.saveGame) {
            window.game.saveGame();
        }
...
console.log("📦 EquipmentSystem модуль загружен с исправленной системой экипировки");
