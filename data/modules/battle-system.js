...
class BattleSystem {
            ...
            <div class="enemy-panel-mini" data-monster-id="${monster.data.battleId}">
                ...
                <div class="enemy-panel-stats">
                    <div class="stat-row">
                        ...
                        <span>Комбо: <span id="enemyCombo-${monster.data.battleId}">${monster.data.combo && monster.data.combo.count > 0 ? `${this.getActionName(monster.data.combo.type)} x${monster.data.combo.count}` : 'Нет'}</span></span>
                    </div>
                </div>
                <div class="enemy-action-history">
                    ...
                    <div class="history-entries" id="enemyHistory-${monster.data.battleId}">
                        ${monster.data.previousActions && monster.data.previousActions.length > 0 ? 
    ...
    executePlayerAction(action) {
        ...
        if (player.combo && player.combo.type === action && player.combo.count < 4) {
            player.combo.count++;
        } else {
            player.combo = { type: action, count: 1 };
        }

        player.currentAction = action;
        player.previousActions = player.previousActions || [];
    ...
    executeNextMonsterTurn(index, monsters) {
        ...
        if (monster.combo && monster.combo.type === action && monster.combo.count < 4) {
            monster.combo.count++;
        } else {
            monster.combo = { type: action, count: 1 };
        }

        monster.previousActions = monster.previousActions || [];
    ...
    updateMonsterPanel(monsterId) {
        ...
        if (comboElement) {
            comboElement.textContent = monster.combo && monster.combo.count > 0 ? 
                `${this.getActionName(monster.combo.type)} x${monster.combo.count}` : 'Нет';
        }
        if (historyElement) {
            historyElement.innerHTML = monster.previousActions && monster.previousActions.length > 0 ? 
    ...
    updateTacticalUI() {
        ...
        if (playerCombo) {
            if (this.players[1].combo && this.players[1].combo.count > 0) {
...
class AdvancedTacticalAI {
    ...
    decideAction() {
        try {
            ...
            console.error(`❌ Ошибка в ИИ ${this.monster.name}:`, error);
    ...
    analyzeBattleContext() {
        ...
        const monsterUnit = this.bs.battleGrid.enemies.find(u => 
            u && u.data && u.data.battleId === this.monster.battleId
        );
        
        // ОБНОВЛЯЕМ ИНФОРМАЦИЮ О ИГРОКЕ
        const player = this.bs.players[1];
        this.lastPlayerAction = player.currentAction;
        this.playerComboCount = player.combo ? player.combo.count : 0;
        this.playerComboType = player.combo ? player.combo.type : null;
        ...
        return {
            ...
            monsterCombo: this.monster.combo || { type: null, count: 0 },
            
            // 🎯 СОСТОЯНИЕ ГЕРОЯ
            heroHPPercent: hero ? hero.currentHealth / hero.maxHealth : 1,
    ...
    doesPlayerBlockOften() {
        ...
        const player = this.bs.players[1];
        const recentActions = player.previousActions ? player.previousActions.slice(0, 3) : [];
    ...
    estimateDamage(action) {
        ...
        const comboCount = this.monster.combo ? this.monster.combo.count : 0;
        const comboMultiplier = this.bs.getComboMultiplier(action, comboCount);
    ...
    predictPlayerAction() {
        ...
        if (!hero) return 'attack';
        
        if (hero.currentHealth < hero.maxHealth * 0.3) {
            return 'heal'; // Игрок вероятно будет лечиться
        }
        
        if (player.combo && player.combo.count >= 3 && player.combo.type === 'block') {
    ...
    selectBestAction(actionScores, availableActions) {
        ...
        if (totalScore === 0) {
            return availableActions[Math.floor(Math.random() * availableActions.length)]; // случайный fallback
        }
