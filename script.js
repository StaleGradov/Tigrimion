...
class HeroGame {
    ...
    // Обновляем рендер выбора локации
    ...
    // Обновляем завершение боя для отслеживания прогресса
    endBattle(victory) {
        if (victory) {
    ...
    // Новая функция для обновления прогресса локации
    ...
    // Обновляем загрузку сохранения для прогресса локаций
    loadSave() {
        try {
            ...
            if (save) {
                ...
                // ... существующий код загрузки героев
                
                // Загружаем прогресс локаций
                if (data.locationProgress) {
                    this.locationProgress = data.locationProgress;
                }
    ...
    // Обновляем сохранение для прогресса локаций
    saveGame() {
        if (this.currentHero) {
            localStorage.setItem('heroGameSave', JSON.stringify({
                ...
                locationProgress: this.locationProgress // Сохраняем прогресс
            }));
        }
    }
}
