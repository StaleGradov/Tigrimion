...
HeroGame.prototype.getActiveSetBonuses = function(hero) {
    ...
    Object.keys(setCounts).forEach(setName => {
        const setConfigItem = setConfig[setName];
        if (setConfigItem && setCounts[setName] >= setConfigItem.requiredPieces) {
            activeSetBonuses.push({
                setName: setName,
                pieces: setCounts[setName],
                bonus: setConfigItem.bonus,
                description: setConfigItem.description
            });
        }
    });
